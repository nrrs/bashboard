...
class Add extends Component {
  ...
  componentWillReceiveProps(newProps) {
    this.setState({subscriptions: newProps.subreddits.subscriptions, results: newProps.subreddits.searchResults});
  }
  ...
  handleSubmit(sub) {
    this
      .props
      .receiveSubs(this.state.subscriptions.concat([sub]));
    this.clear();
  }
    ...
    return (
      <div className="Add" style={addStyles}>
        <h1 style={{
          ...
          <button id="add" onClick={() => this.handleSubmit(this.state.query)}>ADD</button>
        </h1>
        <ul className="results" style={resultStyles}>
          {this
            ...
            .map(result => {
              return (
                <li key={result.id} onClick={() => this.handleSubmit(result.name)}>{result.name}</li>
              );
            })}
        </ul>
      </div>
